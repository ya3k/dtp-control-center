...
import { adminLinks, operatorLinks } from "@/configs/routes";
import {
  Form,
  FormControl,
  FormField,
...
import TokenManager from "@/components/notification/TokenManager";
...
export function LoginForm({
  ...
  // Update form value when fcmToken changes
  useEffect(() => {
    if (fcmToken !== null) {
      form.setValue('fcmToken', fcmToken);
    }
  }, [fcmToken, form]);

  // Handle FCM token update
  const handleTokenReady = (token: string | null) => {
    setFcmToken(token);
    // Also update the form value directly
    form.setValue('fcmToken', token);
  };

  const onSubmit = async (values: LoginSchemaType) => {
    try {
      setLoading(true);
      console.log('Submitting login with FCM token:', values.fcmToken);
  ...
  return (
    <>
      {/* TokenManager to get FCM token */}
      <TokenManager onTokenReady={handleTokenReady} />
