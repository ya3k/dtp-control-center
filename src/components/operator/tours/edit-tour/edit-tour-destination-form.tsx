...
function EditTourDestinationForm({ tourId, onSaveSuccess }: EditTourDestinationFormProps) {
  ...
  const tourDestinationSchema = z.object({
    destinations: z.array(z.object({
      ...
      img: z.string().or(z.array(z.string())),
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Convert the data to match our form type
        const formattedData = tourDestinationsData.map((dest: TourDestinationResType) => ({
          ...dest,
          img: typeof dest.img === 'string' ? dest.img : '',
        }));
        
        // Don't use setTourDestinations to avoid unnecessary re-renders
        // setTourDestinations(tourDestinationsData);
  ...
  const handleDestinationChange = (
    ...
    // Use a controlled update to prevent loops - type more precisely
    form.setValue(`destinations.${destinationIndex}.${field}`, value, { 
      shouldDirty: true,
      shouldTouch: true
    });
  ...
  const handleFileChange = (destinationId: string, event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files.length > 0) {
      ...
      const totalImagesAfterAdd = currentPendingImages.length + files.length;
      
      if (totalImagesAfterAdd > MAX_IMAGES) {
        toast.error(`Không thể tải lên hơn ${MAX_IMAGES} hình ảnh cho mỗi điểm đến.`);
      ...
      setPendingImages(prev => ({
        ...prev,
        [destinationId]: [...(prev[destinationId] || []), ...files]
      }));
    }
  };
    ...
    try {
      ...
      const requestBody = {
        ...
        destinations: sortedDestinations.map(destination => {
          // Prepare the img field - either use pendingImages or existing img as array
          let imgField: string[] = [];
          if (pendingImages[destination.id] && pendingImages[destination.id].length > 0) {
            // For pending images, use the files directly
            imgField = pendingImages[destination.id];
          } else if (typeof destination.img === 'string' && destination.img) {
            // For existing string image, convert to array
            imgField = [destination.img];
      ...
      console.log("Updating tour destinations:", JSON.stringify(requestBody, null, 2));
                                              ...
                                              <div key={`pending-${fileIdx}`} className="relative w-24 h-24 rounded overflow-hidden">
                                                ...
                                                <Button
                                                  ...
                                                  onClick={() => {
                                                    ...
                                                    removePendingImage(destination.id, fileIdx);
                                                  }}
                                                >
                                                  ×
                                                </Button>
